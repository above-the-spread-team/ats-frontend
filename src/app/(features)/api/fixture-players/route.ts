...
const API_URL =
  (process.env.FOOTBALL_API_URL ||
    ...
    DEFAULT_API_URL) + "/fixtures/players";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Fixture Players API Error:", errorText);
      return NextResponse.json(
        {
          error: `Failed to fetch fixture players: ${response.statusText}`,
        },
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unknown error occurred while fetching fixture players",
      },
      { status: 500 }
    );
  }
