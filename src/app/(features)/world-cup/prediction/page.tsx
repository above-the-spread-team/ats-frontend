  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 lg:p-5 space-y-2.5 lg:space-y-3">
      <div className="flex items-center gap-2 pb-1 border-b border-border/60">
        ...
        {localPick &&
          (() => {
            const picked = group.teams.find((t) => t.id === localPick);
            return picked ? (
              <span className="ml-auto text-[10px] lg:text-xs font-semibold text-primary-font truncate max-w-[100px] lg:max-w-[130px]">
                ✓ {picked.name}
              </span>
            ) : null;
          })()}
      ...
      {group.teams.map((team) => {
        ...
        return (
          <div key={team.id} className="space-y-1">
            <div className="flex items-center gap-2 rounded-md px-1.5 py-1 -mx-1.5">
              ...
              {isSelected && (
                <CheckIcon className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-primary-font flex-shrink-0" />
              )}
            ...
            return (
              <div key={team.id} className="space-y-1">
                <div className="flex items-center gap-2 rounded-md px-1.5 py-1 -mx-1.5">
                  ...
                  {isCurrent && (
                    <CheckIcon className="w-3 h-3 lg:w-3.5 lg:h-3.5 text-red-500 flex-shrink-0" />
                  )}
                  <span className="text-[10px] lg:text-xs tabular-nums flex-shrink-0 text-muted-foreground">
                    {pct.toFixed(1)}%
                  </span>
...
function VotingModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      {/* Modal panel */}
      <div className="relative z-10 w-full h-[100dvh] sm:h-auto sm:max-h-[90vh] sm:max-w-[560px] sm:rounded-2xl bg-card flex flex-col overflow-hidden shadow-2xl">
        {/* ── Header ── */}
        ...
        <div className="mx-5 h-1 rounded-full bg-muted overflow-hidden flex-shrink-0">
          <div
            className="h-full rounded-full bg-primary-hero transition-all duration-300"
                      ...
                      <div className="h-1 rounded-full bg-muted overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${
                            isSelected ? "bg-primary-hero" : "bg-primary-hero"
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
              ...
              <div className="px-5 pt-4 pb-2 space-y-3">
                ...
                {champId != null &&
                  (() => {
                    ...
                    if (!team) return null;
                    return (
                      <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-primary-font border border-primary-hero/30">
                        <TeamLogo
                          src={team.logo_url}
                          name={team.name}
                          size={32}
                        />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-primary-white truncate">
                            {team.name}
                          </p>
                        ...
                        <CheckIcon className="w-4 h-4 text-primary-whiteflex-shrink-0" />
                      </div>
                    );
                  })()}
                  ...
                  {filteredTeams.map((team) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[11px] text-white/80 flex-shrink-0">
        ...
        <div className="px-5 pt-3 pb-5 border-t border-border/60 flex-shrink-0 space-y-2">
          ...
          {isChampionStep ? (
            ...
            >
              {displayStep === TOTAL - 2 ? "Next — Pick Champion →" : "Next →"}
            </button>
          )}
        </div>
      </div>
...
export default function WorldCupPredictionPage() {
  ...
  return (
    <div className="px-4 py-6 sm:py-8 pb-16 space-y-5">
      ...
      <div>
        {!isLoggedIn ? (
          <div className="flex flex-col items-center gap-3 py-6 rounded-xl border border-border bg-card text-center px-4">
            <span className="text-3xl">🔐</span>
            <p className="text-sm font-bold">Sign in to vote</p>
            <p className="text-xs text-muted-foreground">
              Create an account or log in to submit your World Cup predictions.
            </p>
          </div>
        ) : votingClosed ? (
