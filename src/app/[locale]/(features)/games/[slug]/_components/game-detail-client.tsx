...
import FixtureDetail from "@/app/[locale]/(features)/games/detail/_components/fixture-detail";
import Lineups from "@/app/[locale]/(features)/games/detail/_components/lineups";
import FixtureStatistics from "@/app/[locale]/(features)/games/detail/_components/fixture-stats";
import Events from "@/app/[locale]/(features)/games/detail/_components/events";
import FixturePlayers from "@/app/[locale]/(features)/games/detail/_components/fixture-players";
import Predictions from "@/app/[locale]/(features)/games/detail/_components/tips";
import Odds from "@/app/[locale]/(features)/games/detail/_components/odds";
...
interface GameDetailClientProps {
  /** API-Football fixture id, parsed from the /games/{slug}-{id} route segment */
  fixtureId: number;
  /** Fixture fetched by the server page — seeds React Query so the first paint has data */
  initialFixture: FixtureResponseItem;
  /** When the server rendered it (ms epoch) */
  initialFixtureUpdatedAt?: number;
}

function GameDetailContent({
  fixtureId,
  initialFixture,
  initialFixtureUpdatedAt,
  ...
  const {
    ...
    initialDataUpdatedAt: initialFixtureUpdatedAt,
  });
  ...
  // Handle error state — only when there is nothing to show. A failed background
  // refetch keeps the fixture we already have on screen.
  const fixture = fixtureData?.response?.[0];
  ...
  return (
    <FullPage minusHeight={10}>
      ...
      {/* Tab Navigation — Nav pushes the current pathname (/games/{slug}) with the
          existing query (?date=) plus ?tab=; the fixture id lives in the path. */}
      <Nav
        tabs={tabs}
        activeTab={activeTab}
        setActiveTab={(tab) => {
...
export default function GameDetailClient(props: GameDetailClientProps) {
  // useSearchParams (?date= / ?tab=) needs a Suspense boundary
  return (
