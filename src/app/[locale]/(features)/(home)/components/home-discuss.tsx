...
import { mapPostResponse } from "@/app/[locale]/(features)/discuss/_components/post-card";
  ...
  if (error) {
    return (
      <div className="w-full">
        ...
        >
          <h2 className="text-lg md:text-xl font-bold">{t("latestDiscussions")}</h2>
        </Link>
        <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
          <HomeFailToLoad message={t("failedToLoadDiscussions")} />
        </div>
      </div>
    );
  }
...
<h2 className="text-lg md:text-xl font-bold">{t("latestDiscussions")}</h2>
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
          <div className="p-4 text-center">
            <p className="text-sm text-muted-foreground">{t("noPostsAvailable")}</p>
          </div>
        </div>
      </div>
    );
