...
function LeaderboardTable({ timeRange }: { timeRange: LeaderboardTimeRange }) {
  ...
  if (error || !data) {
    return <HomeFailToLoad message={lt("couldNotLoad")} />;
  }
      ...
      return (
        <div className="py-6 flex flex-col items-center gap-4 text-center">
          ...
          <div className="w-full max-w-xs md:max-w-md rounded-xl border border-amber-400/20 bg-amber-400/5 px-4 py-3 space-y-2 text-left">
            {[
              { step: "1", text: lt("voteEveryDay") },
              { step: "2", text: lt("hitMinGames") },
              { step: "3", text: lt("stayTop") },
            ].map(({ step, text }) => (
    ...
    return (
      <div className="py-12 text-center space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          {lt("noQualifiedPlayers")}
        </p>
        <p className="text-xs text-muted-foreground/60">
          {lt("needResolvedGames")}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="-mx-4 md:mx-0 px-1 overflow-x-auto scrollbar-hide">
      <div className="flex gap-1 bg-muted/50 p-1 rounded-lg w-max">
        {TABS.map(({ value, wc }) => {
          const label = value === "world_cup_2026"
            ? lt("worldCup2026")
            : value === "overall"
              ? lt("tabOverall")
              : value === "monthly"
