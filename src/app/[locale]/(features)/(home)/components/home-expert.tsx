...
function ExpertAvatar({
  ...
  const displayName = name || at("badges.expert");
...
export default function HomeExpert() {
  ...
  function getFirstTag(news: NewsResponse) {
    return news.tags && news.tags.length > 0 ? news.tags[0].name : at("badges.expert");
  }
  ...
  if (error) {
    return (
      <section className="w-full">
        ...
        >
          <h2 className="text-lg md:text-xl font-bold">{t("expertPerspectives")}</h2>
        </Link>
        <div className="bg-card border border-border rounded-lg overflow-hidden shadow-sm">
          <HomeFailToLoad message={t("failedToLoadExpert")} />
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="w-full">
      <Link
        ...
        className="text-sm px-2 block mb-1.5 text-primary-font hover:underline font-semibold"
      >
        <h2 className="text-base md:text-lg font-bold">{t("ourPicks")}</h2>
      </Link>
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-0.5 flex items-center gap-1.5">
                  <span className="truncate text-xs font-semibold text-amber-600 dark:text-amber-400">
                    {article.expert_name || at("badges.expert")}
                  </span>
                  <Tag
